(() => {
  ...
  Object.assign(ui, {
    ...
    teachOpen:false, teachStartCount:0, teachIndex:0, teachResumeAfterDraw:false
  });
  const TEACH_TYPES=[
    {kind:"table",type:"rectangle",label:"Rectangle Table"},
    {kind:"table",type:"round",label:"Round Table"},
    {kind:"table",type:"square",label:"Square Table"},
    {kind:"table",type:"bistro",label:"Bistro Table"},
    {kind:"venue",type:"chair",label:"Chair"},
    {kind:"venue",type:"stage",label:"Stage"},
    {kind:"venue",type:"bar",label:"Bar"},
    {kind:"venue",type:"entrance",label:"Entrance"},
    {kind:"venue",type:"exit",label:"Exit"},
    {kind:"venue",type:"column",label:"Column"},
    {kind:"venue",type:"other",label:"Other Object"}
  ];
  ...
  function v8Toolbar(event,seating=false){
    const n=ui.selectedObjectIds.length;
    return`<div class="canvas-toolbar v8-toolbar"><div class="tool-group">${toolbarBtn("mouse","Select",`data-tool="select"`,ui.tool==="select")}${toolbarBtn("hand","Pan",`data-tool="pan"`,ui.tool==="pan")}</div>${!seating?`<div class="tool-group">${toolbarBtn("plus","Add / Bulk",`data-v8-action="add"`,ui.v8AddOpen)}${toolbarBtn("copy","Duplicate",`data-v8-action="duplicate-selection"`)}${toolbarBtn("trash","Delete",`data-v8-action="delete-selection"`)}</div><div class="tool-group">${toolbarBtn("undo","Undo",`data-canvas-action="undo"`)}${toolbarBtn("redo","Redo",`data-canvas-action="redo"`)}</div>${n>=2?`<div class="tool-group">${[["alignLeft","Align left","align-left"],["alignCenterH","Align center","align-hcenter"],["alignRight","Align right","align-right"],["alignTop","Align top","align-top"],["alignMiddle","Align middle","align-vcenter"],["alignBottom","Align bottom","align-bottom"]].map(([ic,label,act])=>toolbarBtn(ic,label,`data-v8-action="${act}"`).replace('class="toolbar-btn','class="toolbar-btn icon-only')).join("")}${n>=3?[["distributeH","Distribute horizontally","distribute-h"],["distributeV","Distribute vertically","distribute-v"]].map(([ic,label,act])=>toolbarBtn(ic,label,`data-v8-action="${act}"`).replace('class="toolbar-btn','class="toolbar-btn icon-only')).join(""):""}</div>`:""}`:""}<div class="tool-group">${toolbarBtn("zoomOut","Zoom out",`data-canvas-action="zoom-out"`)}<span class="zoom-label">${Math.round(ui.zoom*100)}%</span>${toolbarBtn("zoomIn","Zoom in",`data-canvas-action="zoom-in"`)}${toolbarBtn("fit","Fit",`data-canvas-action="fit"`)}</div><div class="tool-group">${toolbarBtn("grid","Grid",`data-canvas-action="grid"`,ui.grid)}${toolbarBtn("magnet","Snap",`data-canvas-action="snap"`,ui.snap)}${toolbarBtn("seat","Seat labels",`data-canvas-action="seat-numbers"`,ui.showSeats)}</div><span class="toolbar-spacer"></span>${!seating?toolbarBtn("image","Assisted Detection",`data-v8-action="detect" ${event.background?.src?"":"disabled"}`,false).replace('class="toolbar-btn','class="toolbar-btn ai'):""}${toolbarBtn("fit","Focus Mode",`data-v8-action="focus"`,ui.focusMode)}</div>`;
  }
  ...
  floorPlanHTML = function(event){return`<div class="editor-layout left-collapsed ${ui.rightCollapsed?"right-collapsed":""}">${collapsedPanel("left")}<section class="canvas-column" style="position:relative">${v8Toolbar(event,false)}${bulkPanel(event)}${canvasViewportHTML(event,false)}</section>${inspectorHTML(event)}</div>`;};
  ...
  function selectionObjects(event,ids){return ids.map(id=>event.tables.find(x=>x.id===id)||event.venueObjects.find(x=>x.id===id)).filter(Boolean);}
  function alignSelection(mode){
    const event=activeEvent();if(!canMutate(event,"align plan objects"))return;
    const objs=selectionObjects(event,ui.selectedObjectIds);if(objs.length<2)return toast("Select two or more objects to align.","error");
    if(objs.some(o=>o.locked))return toast("Unlock every selected object before arranging.","error");
    recordUndo(event);
    const minX=Math.min(...objs.map(o=>o.x)),maxRight=Math.max(...objs.map(o=>o.x+o.w)),minY=Math.min(...objs.map(o=>o.y)),maxBottom=Math.max(...objs.map(o=>o.y+o.h)),cx=(minX+maxRight)/2,cy=(minY+maxBottom)/2;
    objs.forEach(o=>{
      if(mode==="left")o.x=minX;else if(mode==="right")o.x=maxRight-o.w;else if(mode==="hcenter")o.x=cx-o.w/2;
      else if(mode==="top")o.y=minY;else if(mode==="bottom")o.y=maxBottom-o.h;else if(mode==="vcenter")o.y=cy-o.h/2;
    });
    touchEvent(event);render();toast(`${objs.length} objects aligned.`,"success");
  }
  function distributeSelection(axis){
    const event=activeEvent();if(!canMutate(event,"distribute plan objects"))return;
    const objs=selectionObjects(event,ui.selectedObjectIds);if(objs.length<3)return toast("Select three or more objects to distribute.","error");
    if(objs.some(o=>o.locked))return toast("Unlock every selected object before arranging.","error");
    recordUndo(event);
    const sorted=[...objs].sort((a,b)=>axis==="h"?a.x-b.x:a.y-b.y),first=sorted[0],last=sorted[sorted.length-1];
    const span=axis==="h"?(last.x+last.w)-first.x:(last.y+last.h)-first.y,totalSize=sorted.reduce((n,o)=>n+(axis==="h"?o.w:o.h),0),gap=(span-totalSize)/(sorted.length-1);
    let cursor=axis==="h"?first.x:first.y;
    sorted.forEach(o=>{if(axis==="h")o.x=cursor;else o.y=cursor;cursor+=(axis==="h"?o.w:o.h)+gap;});
    touchEvent(event);render();toast(`${objs.length} objects distributed evenly.`,"success");
  }
  ...
  bindCanvas = function(){
    ...
    document.querySelectorAll("[data-v8-action]").forEach(button=>button.onclick=()=>{const action=button.dataset.v8Action;if(action==="add"){ui.v8AddOpen=!ui.v8AddOpen;ui.bulkDraft ||= {kind:"table",type:"round",chairs:8,quantity:4,rows:2,cols:2,placement:"grid",prefix:"T",zone:"MAIN FLOOR"};render();}else if(action==="close-add"){ui.v8AddOpen=false;render();}else if(action==="commit-add")commitBulk();else if(action==="duplicate-selection")duplicateSelection();else if(action==="delete-selection")deleteSelection();else if(action==="align-left")alignSelection("left");else if(action==="align-hcenter")alignSelection("hcenter");else if(action==="align-right")alignSelection("right");else if(action==="align-top")alignSelection("top");else if(action==="align-vcenter")alignSelection("vcenter");else if(action==="align-bottom")alignSelection("bottom");else if(action==="distribute-h")distributeSelection("h");else if(action==="distribute-v")distributeSelection("v");else if(action==="focus"){ui.focusMode=!ui.focusMode;render();}else if(action==="detect")runAssistedDetection();});
  ...
  function startListMarquee(container,rowSelector,idAttr,onApply){
    container.addEventListener("pointerdown",e=>{
      if(e.button!==0||e.target!==container)return;e.preventDefault();
      const box=document.createElement("div");box.className="marquee";container.appendChild(box);
      const r=container.getBoundingClientRect(),sx=e.clientX-r.left,sy=e.clientY-r.top;let current=[];
      const move=ev=>{const x=ev.clientX-r.left,y=ev.clientY-r.top,left=Math.min(sx,x),top=Math.min(sy,y),w=Math.abs(x-sx),h=Math.abs(y-sy);Object.assign(box.style,{left:left+"px",top:top+"px",width:w+"px",height:h+"px"});const br={left:r.left+left,top:r.top+top,right:r.left+left+w,bottom:r.top+top+h};current=[...container.querySelectorAll(rowSelector)].filter(el=>{if(!el.dataset[idAttr])return false;const q=el.getBoundingClientRect();return q.right>=br.left&&q.left<=br.right&&q.bottom>=br.top&&q.top<=br.bottom;}).map(el=>el.dataset[idAttr]);};
      const up=ev=>{document.removeEventListener("pointermove",move);document.removeEventListener("pointerup",up);box.remove();if(current.length)onApply(current,ev.ctrlKey||ev.metaKey);};
      document.addEventListener("pointermove",move);document.addEventListener("pointerup",up);
    });
  }
  const oldSelectedTablePanel=selectedTablePanelHTML;
  selectedTablePanelHTML = function(event){
    const html=oldSelectedTablePanel(event);if(ui.selectedGuestIds.length<=1)return html;
    const pax=ui.selectedGuestIds.reduce((n,id)=>{const g=event.guests.find(x=>x.id===id);return n+(g?paxOf(g):0);},0);
    const anchor=`<button class="panel-toggle" data-panel-toggle="right">${icon("chevron")}</button></div>`;
    return html.replace(anchor,`${anchor}<div class="selection-count">${ui.selectedGuestIds.length} record${ui.selectedGuestIds.length===1?"":"s"} selected · ${pax} pax</div>`);
  };
  ...
  bindSeating = function(){
    ...
    document.querySelectorAll("[data-assign-selected]").forEach(b=>b.onclick=()=>{const ids=ui.selectedGuestIds.length?ui.selectedGuestIds:[ui.selectedGuestId].filter(Boolean);if(ids.length)assignGuestGroup(ids,b.dataset.assignSelected);});
    const leftList=document.querySelector(".unassigned-list");if(leftList)startListMarquee(leftList,"[data-seating-guest]","seatingGuest",(ids,union)=>{ui.selectedGuestIds=union?[...new Set([...ui.selectedGuestIds,...ids])]:ids;ui.selectedGuestId=ui.selectedGuestIds[0]||null;ui.guestAnchorId=ids[ids.length-1];render();});
    const rightList=document.querySelector(".seat-list");if(rightList)startListMarquee(rightList,"[data-occupant-guest]","occupantGuest",(ids,union)=>{ui.selectedGuestIds=union?[...new Set([...ui.selectedGuestIds,...ids])]:ids;ui.selectedGuestId=ui.selectedGuestIds[0]||null;ui.guestAnchorId=ids[ids.length-1];render();});
  };
  ...
  async function runAssistedDetection(){
    const event=activeEvent();if(!canMutate(event,"run plan analysis")||!event.background?.src)return toast("Import a floor plan first.","error");ui.analysisBusy=true;ui.analysisProgress=3;ui.analysisStage="Decode high-resolution plan";ui.screen="review";render();await yieldFrame();
    try{
      ...
      ui.analysisStage="Shape, repetition and chair association";ui.analysisProgress=73;render();await yieldFrame();const area=width*height,minDim=Math.min(width,height),notWall=c=>!(Math.min(c.w,c.h)<3&&Math.max(c.w,c.h)>minDim*.08),chairs=components.filter(c=>notWall(c)&&Math.min(c.w,c.h)>=3&&Math.max(c.w,c.h)<=minDim*.065&&c.w*c.h<area*.0028&&c.fill>=.045),tables=components.filter(c=>notWall(c)&&Math.min(c.w,c.h)>=minDim*.016&&c.w*c.h>=area*.00015&&c.w*c.h<=area*.04&&c.aspect>=.35&&c.aspect<=3.8&&c.fill>=.035).sort((a,b)=>b.w*b.h-a.w*a.h).slice(0,100);
      const candidates=tables.map((t,index)=>{const nearby=chairs.filter(ch=>{const cx=ch.x+ch.w/2,cy=ch.y+ch.h/2,margin=Math.max(t.w,t.h)*.65;return cx>=t.x-margin&&cx<=t.x+t.w+margin&&cy>=t.y-margin&&cy<=t.y+t.h+margin&&!(cx>=t.x&&cx<=t.x+t.w&&cy>=t.y&&cy<=t.y+t.h);}).sort((a,b)=>{const ax=a.x+a.w/2-(t.x+t.w/2),ay=a.y+a.h/2-(t.y+t.h/2),bx=b.x+b.w/2-(t.x+t.w/2),by=b.y+b.h/2-(t.y+t.h/2);return ax*ax+ay*ay-bx*bx-by*by;}).slice(0,18),round=t.aspect>.78&&t.aspect<1.28&&t.fill<.68,confidence=Math.min(.94,.4+Math.min(.3,nearby.length*.04)+Math.min(.18,t.fill*.28)),activeThreshold=state.calibration?.recommendedConfidence??.48;return{id:uid("candidate"),kind:"table",type:round?"round":"rectangle",x:t.x/width*100,y:t.y/height*100,w:t.w/width*100,h:t.h/height*100,rotation:t.rotation,confidence,status:"unreviewed",selected:confidence>=activeThreshold,chairDetections:nearby.map(ch=>({id:uid("candidate-chair"),x:(ch.x+ch.w/2)/width*100,y:(ch.y+ch.h/2)/height*100,w:ch.w/width*100,h:ch.h/height*100,rotation:ch.rotation,confidence:.56})),evidence:{geometry:Number(Math.min(.95,t.fill+.35).toFixed(2)),chairs:nearby.length,repetition:tables.filter(o=>Math.abs(o.w-t.w)<t.w*.2&&Math.abs(o.h-t.h)<t.h*.2).length}};});
  ...
  function candidateStatusClass(c){
    if(c.status==="confirmed")return"status-accepted";if(c.status==="rejected")return"status-rejected";if(c.status==="ignored")return"status-ignored";
    const threshold=activeEvent()?.analysis?.confidenceThreshold??.48;return c.confidence>=threshold?"status-accepted":"status-review";
  }
  function candidateBox(c,selected){return`<button class="candidate-box ${c.kind} ${candidateStatusClass(c)} ${selected?"selected":""}" data-candidate-box="${c.id}" style="left:${c.x}%;top:${c.y}%;width:${c.w}%;height:${c.h}%;transform:rotate(${c.rotation||0}deg)" title="${esc(c.kind)} · ${Math.round(c.confidence*100)}%"></button>${(c.chairDetections||[]).map(ch=>`<i class="candidate-box chair" style="left:${ch.x}%;top:${ch.y}%;width:${Math.max(.5,ch.w)}%;height:${Math.max(.5,ch.h)}%;transform:translate(-50%,-50%) rotate(${ch.rotation||0}deg)"></i>`).join("")}`;}
  function confirmAllDetections(){
    const event=activeEvent(),a=event.analysis;if(!a)return;
    const eligible=a.candidates.filter(c=>c.status!=="rejected"&&c.status!=="ignored");
    if(!eligible.length)return toast("No eligible detections to confirm. Rejected and ignored items are preserved as-is.","error");
    const lowConfidence=eligible.filter(c=>c.confidence<(a.confidenceThreshold??.48)).length;
    eligible.forEach(c=>{c.selected=true;});
    if(lowConfidence)toast(`Confirm All includes ${lowConfidence} lower-confidence detection${lowConfidence===1?"":"s"} that manual selection would have skipped.`,"success",6000);
    commitCandidates();
  }
  function analysisHTML(event){
    const a=event.analysis,candidates=reviewCandidates(event),selected=a?.candidates.find(c=>c.id===ui.selectedCandidateId);
    const needHelp=a?teachQueue(event).length:0;
    return`<section class="analysis-screen"><header class="analysis-head"><button class="btn" data-review-action="back">${icon("arrow")}Floor Plan</button><div><h2>Visual Review · Assisted Detection</h2><p>${a?.notice||"No objects are created until you confirm detections."}</p></div><div class="analysis-progress"><div class="analysis-progress-track"><i style="width:${ui.analysisProgress}%"></i></div><small>${esc(ui.analysisStage)} · ${ui.analysisProgress}%</small></div><span class="toolbar-spacer"></span>${a?`<button class="btn" data-review-action="reanalyze">Re-Analyze</button><button class="btn" data-review-action="confirm-all">Confirm All Detections</button><button class="btn primary" data-review-action="commit">Confirm Selected to Plan</button>`:""}</header>${a?`<div class="analysis-grid"><aside class="analysis-rail"><div class="analysis-note">${esc(a.engine)} · ${a.diagnostics.resolution} · Confidence threshold ${Math.round((a.confidenceThreshold??.48)*100)}%${state.calibration?` (from local calibration v${state.calibration.version})`:" (default)"}<br>Diff +${a.comparison.added} / −${a.comparison.removed} / Δ${a.comparison.changed}</div><div class="analysis-toolbar"><div class="field"><label>Status</label><select data-review-filter="status"><option value="all">All</option>${["unreviewed","confirmed","rejected","ignored"].map(v=>`<option ${ui.reviewFilter===v?"selected":""}>${v}</option>`).join("")}</select></div><div class="field"><label>Class</label><select data-review-filter="class"><option value="all">All</option><option value="table" ${ui.reviewClass==="table"?"selected":""}>table</option><option value="venue" ${ui.reviewClass==="venue"?"selected":""}>venue</option></select></div><div class="field full"><label>Minimum confidence ${Math.round(ui.reviewConfidence*100)}%</label><input data-review-filter="confidence" type="range" min="0" max=".95" step=".05" value="${ui.reviewConfidence}"></div></div><div class="candidate-list">${candidates.map(c=>`<div class="candidate-card ${selected?.id===c.id?"active":""}" data-candidate="${c.id}"><input type="checkbox" data-candidate-select="${c.id}" ${c.selected?"checked":""}><div><b><i class="candidate-dot ${candidateStatusClass(c)}"></i>${esc(c.type)} · ${c.kind}</b><span>${(c.chairDetections||[]).length} associated chairs · ${Math.round(c.rotation||0)}°</span></div><em>${Math.round(c.confidence*100)}%</em></div>`).join("")||`<div class="inspector-empty">No detections match this filter.</div>`}</div></aside><main class="analysis-canvas-shell"><div class="analysis-scene ${ui.reviewDrawMode?"draw-mode":""}" id="analysisScene"><img src="${event.background.src}" alt="Floor plan analysis source">${candidates.map(c=>candidateBox(c,selected?.id===c.id)).join("")}</div></main><aside class="analysis-rail right"><div class="analysis-detail">${selected?`<h3>Detection Inspector</h3><div class="field"><label>Class</label><select data-candidate-edit="kind"><option value="table" ${selected.kind==="table"?"selected":""}>Table</option><option value="venue" ${selected.kind==="venue"?"selected":""}>Venue</option></select></div><div class="field"><label>Type</label><input data-candidate-edit="type" value="${esc(selected.type)}"></div><div class="field"><label>Rotation</label><input data-candidate-edit="rotation" type="number" value="${Math.round(selected.rotation||0)}"></div><div class="field"><label>Physical chairs</label><input data-candidate-edit="chairs" type="number" min="0" max="99" value="${selected.chairDetections?.length||0}"></div><dl><dt>Confidence</dt><dd>${Math.round(selected.confidence*100)}%</dd><dt>Geometry</dt><dd>${selected.evidence.geometry}</dd><dt>Repetition</dt><dd>${selected.evidence.repetition}</dd></dl><div class="analysis-actions"><button class="btn sm primary" data-review-action="confirm">Confirm</button><button class="btn sm danger" data-review-action="reject">Reject</button><button class="btn sm" data-review-action="ignore">Ignore</button><button class="btn sm full" data-review-action="delete-candidate">Remove detection</button></div>`:`<div class="inspector-empty">Select a candidate to inspect and correct it.</div>`}</div><div class="teach-box"><h4>Teach AI</h4><p>Review the objects Assisted Detection wasn't sure about, one at a time, in plain language — no class IDs, no jargon.</p><button class="btn primary full" style="width:100%" data-review-action="teach-open">${needHelp?`${needHelp} Object${needHelp===1?"":"s"} Need Your Help`:"Nothing Currently Needs Help"}</button><div class="teach-box-secondary"><label><input type="checkbox" data-teach-ai ${ui.teachAI?"checked":""}>Teach AI with corrections</label><button class="btn sm" data-review-action="draw">${ui.reviewDrawMode?"Cancel Drawing":"Draw Missed Detection"}</button><button class="btn sm" data-review-action="save-verified">Save Verified Plan</button><button class="btn sm" data-review-action="improve">Improve AI (Local Calibration)</button></div></div></aside></div>${ui.teachOpen?teachModalHTML(event):""}`:`<div class="v8-empty" style="margin:40px"><h2>${ui.analysisBusy?"Analyzing locally…":"No analysis yet"}</h2><p>${esc(ui.analysisStage)}</p></div>`}</section>`;
  }
  ...
  function commitCandidates(){
    const event=activeEvent(),chosen=event.analysis?.candidates.filter(c=>c.selected&&c.status!=="rejected"&&c.status!=="ignored")||[];if(!chosen.length)return toast("Select at least one detection to confirm.","error");recordUndo(event);let tables=0,venues=0;for(const c of chosen){if(c.committedId)continue;const x=c.x/100*WORLD.width,y=c.y/100*WORLD.height,w=Math.max(55,c.w/100*WORLD.width),h=Math.max(45,c.h/100*WORLD.height);if(c.kind==="table"){const table=syncTableChairs({id:uid("table"),number:uniqueNumber(event,"T",1),type:["round","square","rectangle","bistro"].includes(c.type)?c.type:"rectangle",x,y,w,h,capacity:Math.max(1,c.chairDetections?.length||1),zone:"MAIN FLOOR",rotation:c.rotation||0,locked:false,z:10,hasPhysicalSeats:true});if(c.chairDetections?.length){table.chairs=c.chairDetections.map((ch,index)=>({id:uid("chair"),parentTableId:table.id,seatNumber:index+1,x:Math.max(0,Math.min(100,(ch.x-c.x)/c.w*100)),y:Math.max(0,Math.min(100,(ch.y-c.y)/c.h*100)),rotation:ch.rotation||0,occupancy:null}));table.capacity=table.chairs.length;}event.tables.push(table);c.committedId=table.id;tables++;}else{const object={id:uid("venue"),type:c.type||"text",label:String(c.type||"OBJECT").toUpperCase(),x,y,w,h,rotation:c.rotation||0,locked:false,z:4};event.venueObjects.push(object);c.committedId=object.id;venues++;}c.status="confirmed";}touchEvent(event);ui.screen="workspace";ui.tab="floor";render();toast(`${tables} table${tables===1?"":"s"}, ${venues} venue object${venues===1?"":"s"} confirmed. Chair coordinates were preserved.`,"success",6000);
  }
  function bindReviewDrawing(){const scene=document.getElementById("analysisScene");if(!scene||!ui.reviewDrawMode)return;scene.onpointerdown=e=>{if(e.target!==scene&&e.target.tagName!=="IMG")return;e.preventDefault();const r=scene.getBoundingClientRect(),sx=e.clientX-r.left,sy=e.clientY-r.top,box=document.createElement("div");box.className="candidate-box selected";scene.appendChild(box);const move=ev=>{const x=ev.clientX-r.left,y=ev.clientY-r.top;Object.assign(box.style,{left:Math.min(sx,x)+"px",top:Math.min(sy,y)+"px",width:Math.abs(x-sx)+"px",height:Math.abs(y-sy)+"px"});};const up=ev=>{document.removeEventListener("pointermove",move);document.removeEventListener("pointerup",up);const x=Math.min(sx,ev.clientX-r.left)/r.width*100,y=Math.min(sy,ev.clientY-r.top)/r.height*100,w=Math.abs(ev.clientX-r.left-sx)/r.width*100,h=Math.abs(ev.clientY-r.top-sy)/r.height*100;if(w>1&&h>1){const c={id:uid("candidate"),kind:"table",type:"rectangle",x,y,w,h,rotation:0,confidence:1,status:"unreviewed",selected:true,missed:true,chairDetections:[],evidence:{geometry:"manual",chairs:0,repetition:0}};activeEvent().analysis.candidates.push(c);activeEvent().analysis.missed.push(c.id);ui.selectedCandidateId=c.id;ui.reviewDrawMode=false;if(ui.teachResumeAfterDraw){ui.teachResumeAfterDraw=false;ui.teachOpen=true;ui.teachStartCount=(ui.teachStartCount||0)+1;}touchEvent(activeEvent());}render();};document.addEventListener("pointermove",move);document.addEventListener("pointerup",up);};}
  function saveVerified(){const event=activeEvent();if(!ui.teachAI)return toast("Enable Teach AI with corrections first.","error");const a=event.analysis;if(!a)return;state.verifiedExamples.push({id:uid("verified"),eventId:event.id,savedAt:nowISO(),engine:a.engine,trainedModel:false,threshold:a.threshold,confidenceThreshold:a.confidenceThreshold??.48,imageSize:[a.imageWidth,a.imageHeight],predictions:a.candidates.map(clone),groundTruth:a.candidates.filter(c=>c.status==="confirmed").map(clone),rejected:a.candidates.filter(c=>c.status==="rejected").map(c=>c.id),ignored:a.candidates.filter(c=>c.status==="ignored").map(c=>c.id),missed:[...a.missed],hardExample:a.missed.length>0||a.candidates.some(c=>c.status==="rejected"||c.status==="ignored")});saveState();toast("Verified plan saved locally with predictions, corrections, rejections and ignored/missed detections.","success",6000);}
  function teachQueue(event){return(event.analysis?.candidates||[]).filter(c=>c.status==="unreviewed");}
  function teachCropRegion(c,event){const pad=Math.max(c.w,c.h)*.6,x0=Math.max(0,c.x-pad),y0=Math.max(0,c.y-pad),x1=Math.min(100,c.x+c.w+pad),y1=Math.min(100,c.y+c.h+pad),w=Math.max(1,x1-x0),h=Math.max(1,y1-y0),imgW=event?.analysis?.imageWidth||1,imgH=event?.analysis?.imageHeight||1;return{x0,y0,w,h,aspect:Number(((w*imgW)/(h*imgH)).toFixed(4))};}
  function openTeachAI(){const event=activeEvent();if(!canMutate(event,"teach the detector"))return;const queue=teachQueue(event);if(!queue.length)return toast("No detections currently need your help.","success");ui.teachAI=true;ui.teachOpen=true;ui.teachStartCount=queue.length;ui.teachIndex=0;ui.selectedCandidateId=queue[0].id;render();}
  function closeTeachAI(){ui.teachOpen=false;if(ui.teachAI&&activeEvent()?.analysis)saveVerified();render();}
  function teachClassify(c,kind,type){const event=activeEvent();c.kind=kind;c.type=type;c.status="confirmed";c.selected=true;if(kind==="table"&&!(c.chairDetections&&c.chairDetections.length))c.chairDetections=Array.from({length:8},()=>({id:uid("candidate-chair"),x:c.x+c.w/2,y:c.y+c.h/2,w:.7,h:.7,rotation:0,confidence:1}));touchEvent(event);}
  function teachIgnore(c){c.status="ignored";c.selected=false;touchEvent(activeEvent());}
  function teachAdjustChairs(c,delta){const cur=c.chairDetections||[];if(delta>0&&cur.length<40)c.chairDetections=[...cur,{id:uid("candidate-chair"),x:c.x+c.w/2,y:c.y+c.h/2,w:.7,h:.7,rotation:0,confidence:1}];else if(delta<0&&cur.length>0)c.chairDetections=cur.slice(0,-1);touchEvent(activeEvent());}
  function teachNext(){const event=activeEvent(),queue=teachQueue(event);ui.teachIndex=(ui.teachIndex||0)+1;if(!queue.length){closeTeachAI();toast("All flagged objects reviewed. Verified plan saved.","success",6000);return;}ui.selectedCandidateId=queue[0].id;render();}
  function teachModalHTML(event){
    const c=event.analysis.candidates.find(x=>x.id===ui.selectedCandidateId);if(!c)return"";
    const region=teachCropRegion(c,event),shown=(ui.teachIndex||0)+1,total=Math.max(shown,ui.teachStartCount||1),isTable=c.kind==="table";
    return`<div class="modal-layer teach-modal"><div class="modal-card wide teach-card"><div class="dialog-head"><h2>AI Needs Your Help</h2><button class="dialog-close" data-teach-close>${icon("x")}</button></div><div class="teach-progress">Object ${shown} of ${total}</div><div class="teach-body"><div class="teach-crop-frame" id="teachCropFrame" style="aspect-ratio:${region.aspect}"><div class="teach-crop-inner" style="width:${100/region.w*100}%;height:${100/region.h*100}%;left:${-region.x0/region.w*100}%;top:${-region.y0/region.h*100}%"><img src="${event.background.src}" alt="Detected object crop"><div class="teach-crop-box" style="left:${c.x}%;top:${c.y}%;width:${c.w}%;height:${c.h}%;transform:rotate(${c.rotation||0}deg)"></div>${(c.chairDetections||[]).map((ch,i)=>`<i class="teach-chair-marker" data-teach-chair="${i}" style="left:${ch.x}%;top:${ch.y}%"></i>`).join("")}</div></div><div class="teach-side"><p class="teach-question">What is this object?</p><div class="teach-type-grid">${TEACH_TYPES.map(t=>`<button class="teach-type-btn ${c.kind===t.kind&&c.type===t.type?"active":""}" data-teach-type="${t.kind}:${t.type}">${t.label}</button>`).join("")}<button class="teach-type-btn ignore" data-teach-ignore>Ignore / Not Important</button></div>${isTable?`<div class="teach-chair-step"><p>How many chairs?</p><div class="seat-stepper"><button data-teach-chair-step="-1" title="Remove one chair">−</button><b>${c.chairDetections?.length||0}</b><button data-teach-chair-step="1" title="Add one chair">+</button></div><p class="teach-hint">Drag a chair marker on the image to move it.</p></div>`:""}<div class="teach-actions"><button class="btn" data-teach-missed>AI Missed Something Else</button><button class="btn primary" data-teach-next>Save &amp; Next</button></div></div></div></div></div>`;
  }
  function bindTeachChairDrag(c){
    const frame=document.getElementById("teachCropFrame");if(!frame)return;const region=teachCropRegion(c,activeEvent());
    frame.querySelectorAll("[data-teach-chair]").forEach(marker=>{
      marker.onpointerdown=e=>{
        e.preventDefault();e.stopPropagation();const r=frame.getBoundingClientRect(),index=Number(marker.dataset.teachChair);
        const move=ev=>{const px=Math.max(0,Math.min(1,(ev.clientX-r.left)/r.width)),py=Math.max(0,Math.min(1,(ev.clientY-r.top)/r.height)),chair=c.chairDetections[index];if(!chair)return;chair.x=region.x0+px*region.w;chair.y=region.y0+py*region.h;marker.style.left=chair.x+"%";marker.style.top=chair.y+"%";};
        const up=()=>{document.removeEventListener("pointermove",move);document.removeEventListener("pointerup",up);touchEvent(activeEvent());render();};
        document.addEventListener("pointermove",move);document.addEventListener("pointerup",up);
      };
    });
  }
  function bindTeachAI(){
    const event=activeEvent(),c=event.analysis?.candidates.find(x=>x.id===ui.selectedCandidateId);if(!c)return;
    document.querySelector("[data-teach-close]")?.addEventListener("click",closeTeachAI);
    document.querySelectorAll("[data-teach-type]").forEach(b=>b.onclick=()=>{const[kind,type]=b.dataset.teachType.split(":");teachClassify(c,kind,type);if(kind==="table")render();else teachNext();});
    document.querySelector("[data-teach-ignore]")?.addEventListener("click",()=>{teachIgnore(c);teachNext();});
    document.querySelector("[data-teach-missed]")?.addEventListener("click",()=>{ui.teachOpen=false;ui.reviewDrawMode=true;ui.teachResumeAfterDraw=true;render();});
    document.querySelector("[data-teach-next]")?.addEventListener("click",teachNext);
    document.querySelectorAll("[data-teach-chair-step]").forEach(b=>b.onclick=()=>{teachAdjustChairs(c,Number(b.dataset.teachChairStep));render();});
    bindTeachChairDrag(c);
  }
  ...
  window.addEventListener("keydown",e=>{
    if(e.key==="Escape"){if(ui.repeatPlacement){ui.repeatPlacement=null;toast("Repeated placement cancelled.");}if(ui.focusMode)ui.focusMode=false;if(ui.reviewDrawMode){ui.reviewDrawMode=false;ui.teachResumeAfterDraw=false;}if(ui.teachOpen){closeTeachAI();return;}render();return;}
