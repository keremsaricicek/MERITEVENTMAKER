(() => {
  ...
  const railIcons={floor:"grid",guests:"users",seating:"seat",live:"eye",reports:"print"};
  workspaceHTML = function(event){
    ...
    return`<section class="workspace ${ui.focusMode?"v8-focus":""}"><aside class="workspace-rail"><div class="brand-symbol">M✦</div>${tabs.map(([id,label])=>`<button class="rail-btn ${ui.tab===id?"active":""}" data-tab="${id}" title="${label}">${icon(railIcons[id]||"grid")}<span>${label}</span></button>`).join("")}<span class="rail-spacer"></span></aside><div class="workspace-main"><header class="workspace-topbar"><div class="event-id"><strong>${esc(event.name)}</strong><span>${esc(fmtDate(event.date))} · ${esc([event.hotel,event.salon].filter(Boolean).join(" · ")||"Venue not set")}</span></div><div class="workspace-actions"><div class="global-search">${icon("search")}<input id="globalGuestSearch" placeholder="Find guest, inviter, table or seat" autocomplete="off"><div id="globalSearchResults" class="search-results hidden"></div></div>${planHealthHTML(event)}<button class="btn quiet icon-only" data-action="save-now" title="Save now">${icon("save")}</button>${helpButton()}<button class="btn sm" data-action="back-events">All Events</button></div></header>${historical?`<div class="workspace-readonly-banner">${icon("lock")}Historical event · Seating, guest and report records are immutable.</div>`:""}<div class="content">${tabContent(event)}</div></div>${ui.focusMode?`<button class="focus-exit" data-v8-action="focus">Esc · Exit Focus Mode</button>`:""}</section>`;
  };
  ...
  function teachReject(c){c.status="rejected";c.selected=false;touchEvent(activeEvent());}
  ...
  function teachNext(){const event=activeEvent(),queue=teachQueue(event);ui.teachIndex=(ui.teachIndex||0)+1;if(!queue.length){closeTeachAI();toast("All flagged objects reviewed.","success",5000);return;}ui.selectedCandidateId=queue[0].id;render();}
  ...
  function bindTeachAI(){
    ...
    document.querySelector("[data-teach-reject]")?.addEventListener("click",()=>{teachReject(c);teachNext();});
